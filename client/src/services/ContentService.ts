...
export class ContentService implements IContentService {
  ...
  upload = async (file: string): Promise<IEditorModel> => {
    console.log(
      `ContentService: Getting information to edit ${file}...`
    )

    const requestBody = new FormData()
    requestBody.append('file', file)

    const res = await fetch(`${this.baseUrl}/openfile`, {
      method: 'POST',
  ...
  play = async (file: string): Promise<IPlayerModel> => {
    console.log(
      `ContentService: Getting information to play ${file}...`
    )
    const requestBody = new FormData()
    requestBody.append('file', file)

    const res = await fetch(`${this.baseUrl}/playfile`, {
      method: 'POST',
      body: new URLSearchParams(`file=${file}`)
    })
